...
def index():
    ...
    cursor.execute('SELECT * FROM oscars')
...
def record_view(oscar_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM oscars WHERE id=%s', oscar_id)
    result = cursor.fetchall()
...
def form_delete_post(oscar_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM oscars WHERE id = %s """
...
def api_edit(oscar_id) -> str:
    ...
    sql_update_query = """UPDATE oscars t SET t.Index = %s, t.Year = %s, t.Age = %s, t.Name = 
        %s, t.Movie = %s, t.Column_6 = %s WHERE t.id = %s """
    cursor.execute(sql_update_query, inputData)
    mysql.get_db().commit()
